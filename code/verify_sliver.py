...
(recover_sliver_out/, produced by recover_lowdelta.py).  Checks, from disk only:

 (V1) COVERAGE: the set of 'src' top-cell boxes in rec_[0-9]*.jsonl equals EXACTLY the set of
      'stuck' cells of the dk_sliver sweep (dk_sliver/stuck_all.jsonl, or re-extracted from boxes/*.gz).
...
Usage: python verify_recover.py [--outdir recover_sliver_out] [--sample 400] [--full]
"""
...
def load_stuck_set():
    ...
    if os.path.exists(dump):
        ...
        return S
    for fn in glob.glob(os.path.join(here, 'dk_sliver', 'boxes', 'box_*.jsonl.gz')):
...
def _skip_chunk(rows):
    """independently re-verify cylinder corner-skip leaves: re-certify the depth d_lo over each
    cell's (δ,φ) window and confirm the cell lies within the cylinder reach d_lo/(M0*δ_hi) of a
    marginal corner (SOUND: Theorem 8.1 excludes it for every translation). Returns bad count."""
    import dk_sweep as _SW
    from rc_dlo2 import dlo_window2
    from rc_dlo import I as _I
    bad = 0
    for r in rows:
        try:
            cen, hw = r['cen'], r['hw']
            dlo = dlo_window2(_I(cen[0] - hw[0], cen[0] + hw[0]), _I(cen[1] - hw[1], cen[1] + hw[1]))
            if not dlo or dlo <= 0: bad += 1; continue
            reach = dlo / (cen[0] + hw[0])
            if _SW.corner_dist_hi(cen, hw) > reach: bad += 1
        except Exception:
            bad += 1
    return bad

def main():
    outdir = 'recover_sliver_out'; sample = 400; full = False; jobs = 1
    a = sys.argv[1:]
    ...
    tiling_bad = []; unclosed_fail = []; cert_rows = []; skip_rows = []
    for fn in files:
    ...
    # V5: re-verify cylinder corner-skip leaves (depth re-certified + reach check)
    spick = skip_rows if full else rng.sample(skip_rows, min(sample * max(1, len(files)), len(skip_rows)))
    if jobs > 1 and len(spick) > jobs:
        import multiprocessing as mp
        try: mp.set_start_method('fork')
        except RuntimeError: pass
        nch = jobs * 8
        chunks = [spick[i::nch] for i in range(nch)]
        with mp.Pool(jobs) as pool:
            sbad = sum(pool.map(_skip_chunk, chunks))
    else:
        sbad = _skip_chunk(spick)
    v5 = (sbad == 0)
    print('V5 corner-skip re-verify (%s %d of %d skip leaves, jobs=%d): bad=%d  %s'
          % ('FULL' if full else 'sample', len(spick), len(skip_rows), jobs, sbad, 'PASS' if v5 else 'FAIL'))

    verdict = 'PASS' if (v1 and v2 and v3 and v4 and v5) else 'FAIL'
    print('-' * 60); print('RECOVER-LAYER VERDICT:', verdict)
    json.dump(dict(v1=v1, v2=v2, v3=v3, v4=v4, v5=v5, stuck=len(stuck), src=len(srcset),
                   leaves=n_leaf, cert=len(cert_rows), skip=len(skip_rows),
                   unclosed_fail=len(unclosed_fail), verdict=verdict),
              open(os.path.join(outdir, 'verify_sliver_report.json'), 'w'), indent=1)
